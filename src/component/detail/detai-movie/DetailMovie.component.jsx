    ...
    <Container>
      ...
      <Wrap className="detail-video">
        <Wrap className="detail-video-img">
          <img src={videoImg} alt="" onClick={() => setstate(true)} />
            ...
            <Wrap className="producer-list">
              {producerList.map((producer, index) => {
                if (index == producerList.length - 1) {
                  return <span key={index}>{producer}</span>;
                } else {
                  return (
                    <span key={index}>
                      {producer}
...
export default memo(
  withRouter(connect(mapStateToProps, mapDispatchToProps)(DetailMovie))
);
