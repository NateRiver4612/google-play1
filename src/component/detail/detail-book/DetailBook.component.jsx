...
import {Wrap,Container,Span,ButtonContainer,PurchasedIcon,RemoveCartIcon,AddCartButton,AddCartIcon} from './DetailBook.styles'
...
const  DetailBook= ({id,buyItems,addItem,currentUser,signInWithGoggleStart,listItems,removeItem,doc,selectPersonStart,item})=>{
    const {title, imgUrl,price,type,rate,description,author,published,authorImg,audio,publisher,page,language} = item
    ...
    const added = listItems.find(listItem=>
        listItem['item'].id === id && listItem['doc'] === doc   
    )
    const buyed = useMemo(()=>{
         ...
         <Wrap>
            ...
            <Wrap className="detail-info">
                ...
                <Span className='date-author'>
                    <Link to={`/books/participant/${name}`} style={{color:'#0396de',fontWeight:'500'}} classname="author"
                        onClick={()=>{
