...
export const RateBars = ({ selectedComments }) => {
  ...
  const average = useMemo(
    () =>
      (
        (5 * rate5 + 4 * rate4 + 3 * rate3 + 2 * rate2 + 1 * rate1) /
        total
      ).toFixed(1),
    [rate1, rate2, rate3, rate4, rate5, total]
  );

  var remain = average.split(".")[1];
  var number = average.split(".")[0];

  const rate5Percent = useMemo(
    () => Math.floor((rate5 * 100) / total),
    [total]
  );
  const rate4Percent = useMemo(
    () => Math.floor((rate4 * 100) / total),
    [total]
  );
  const rate3Percent = useMemo(
    () => Math.floor((rate3 * 100) / total),
    [total]
  );
  const rate2Percent = useMemo(
    () => Math.floor((rate2 * 100) / total),
    [total]
  );
  const rate1Percent = useMemo(
    () => Math.floor((rate1 * 100) / total),
    [total]
  );
